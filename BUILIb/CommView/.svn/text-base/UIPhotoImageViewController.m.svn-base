...
-(void)didStartLoadCellImageData:(NSIndexPath*)indexPath
{
	...
	if (userIcon != nil)
	{
		/*
         ...
         [cell.photoButton setNeedsDisplay];
         */
        [self.iconDelegate setCellUserIcon:userIcon withIndexPath:indexPath withAnimation:NO];
	}
	else
	{
        [self startDownloadImageUrl:userIconPath withIndexPath:indexPath];
...
-(void)startDownloadImageUrl:(NSString*)imagePath withIndexPath:(NSIndexPath*)indexPath
{
    ...
    if (downloader == nil )
    {
        downloader = [[MemoPhotoDownloader alloc] initWithTinyImageUrl:imagePath indexPath:indexPath];
...
- (void) requestCompleted:(NTESMBRequest *) request
{
	if([request isKindOfClass:[MemoPhotoDownloader class]])
    {
        ...
        [self.iconDelegate setCellUserIcon:image withIndexPath:indexPath withAnimation:YES];
...
{
    NSArray *allDownloaders = [allIconDownloaders allValues];
    for (MemoPhotoDownloader *downlaoder in allDownloaders)
    {
        downlaoder.delegate = nil;
