...
{   
	...
	//self.view = mainView;
    mainView.mainFramView.backgroundColor = kLoginAndSignupCellImageBGColor;
...
- (void)addObservers{
    [ZCSNotficationMgr addObserver:self call:@selector(didNetOK:) msgName:kZCSNetWorkOK];
     ...
     [ZCSNotficationMgr addObserver:self call:@selector(didNetFailed:) msgName:kZCSNetWorkConnectionFailed];
...
- (void)viewDidLoad
{
    ...
    self.view.backgroundColor = kLoginAndSignupCellImageBGColor;
#if 1
    //as the 
    ...
    logInfo.separatorStyle = UITableViewCellSeparatorStyleSingleLine;
    ...
    /*
    cell.layer.borderColor = [kLoginAndSignupCellLineColor CGColor];
...
{
   [self performSelectorOnMainThread:@selector(didLoginOK:) withObject:ntf waitUntilDone:YES];
