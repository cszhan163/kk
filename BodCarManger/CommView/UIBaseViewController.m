...
@synthesize mainContentViewPendingY;
@synthesize mainContentViewPendingX;
@synthesize mainView;
@synthesize delegate;
@synthesize leftBtn;
@synthesize rightBtn;
...
@synthesize mainViewType;
-(id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    if(self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil])
    {
        [self addObservers];
        mainViewType = NEAppFrameViewDefault|NEAppFrameViewBottomBarNone;
        mainContentViewPendingX = kMBAppRealViewXLeftPending;
        mainContentViewPendingY = 0.f;
    }   
    return self;
}
-(void)initTopNavBarViews
...
-(void)initHomePageTimelineNavBar:(CGRect)rect withIndex:(NSInteger)index
{
	 ...
	 */
	UIImageWithFileName(bgImage,@"btn_bak.png");
	//UIImage *bgImageS = nil;
	//UIImageWithFileName(bg
	[btn setBackgroundImage:bgImage forState:UIControlStateNormal];
	UIImageWithFileName(bgImage,@"btn_back_sel.png");
...
-(void)setRightBtnEnable:(BOOL)enable
{
    ...
    {
        rightBtn.enabled = NO;
        rightText.textColor = HexRGB(218, 60, 60);//[UIColor grayColor];
//        rightText.shadowColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];//[UIColor grayColor];
...
-(void)setRightTextContent:(NSString*)text
{
    rightText.text = text;
#ifdef USE_LABEL
    CGRect rect = self.rightText.frame;
    rightText.frame = CGRectMake(rect.origin.x, rect.origin.y,self.rightBtn.frame.size.width, self.rightBtn.frame.size.height);
#endif
}
...
-(void)loadView
{
     ...
     */
	mainView = [[NEAppFrameView alloc]initWithFrame:[[UIScreen mainScreen]bounds] style:mainViewType];
    CGRect rect;
    rect = mainView.mainFramView.frame;
    mainView.mainFramView.frame = CGRectMake(rect.origin.x,rect.origin.y+self.mainContentViewPendingY,rect.size.width,rect.size.height-self.mainContentViewPendingY);
    ...
    mainView.mainFramView.frame =  CGRectMake(rect.origin.x, rect.origin.y-self.mainContentViewPendingY, rect.size.width, rect.size.height);
    */
