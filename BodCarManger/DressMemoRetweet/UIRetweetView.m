...
- (void)layoutSubviews{
    ...
    if ([_image.image isKindOfClass:[UIImage class]]) {
    ...
    CGFloat viewWidth = self.contentView.bounds.size.width-kUIRetweetInputCellLeftPadding-kUIRetweetInputCellLeftPadding;
    
    if ([_image.image isKindOfClass:[UIImage class]]) {
        viewWidth -=_image.bounds.size.width+kUIRetweetInputCellImageInputPadding;
    }
    
    CGFloat pointX = kUIRetweetInputCellLeftPadding+_image.bounds.size.width;
    if ([_image.image isKindOfClass:[UIImage class]]) {
        pointX += kUIRetweetInputCellImageInputPadding;
    }
    
    _inputView.frame = CGRectMake(pointX, 
                                  kUIRetweetInputCellTopPadding, 
