...
- (BOOL)switchState:(NSString *)type{
    ...
    if ([dic isKindOfClass:[NSDictionary class]]) {
        NSMutableDictionary *dataModel = [[[NSMutableDictionary alloc] initWithDictionary:dic] autorelease];
...
/*        
        UIAlertView *tv = [[UIAlertView alloc] initWithTitle:@"分享结果" message:result
                                                    delegate:[_shareSuccessArray count] == 0?nil:self
                                           cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [tv show];
        [tv release];
 */
        if ([_shareFailArray count]) {
            if ([self.delegate respondsToSelector:@selector(centerSendStatusFailed:)]) {
                [self.delegate centerSendStatusFailed:self];
            }
        }else {
            if ([self.delegate respondsToSelector:@selector(centerSendStatusCompleted:)]) {
                [self.delegate centerSendStatusCompleted:self];
            }
        }
...
//    [tView release];
    ...
    if ([engine isKindOfClass:[WeiBoSinaEngine class]] &&
        ([reason isEqualToString:kWeiBoSinaEngineFailReasonNoBind] ||
         [reason isEqualToString:kWeiBoSinaEngineFailReasonExpire])) {
            ...
            engine.tokenKey = nil;
            engine.tokenSecret = nil;
            [self removeModelDataWithType:K_PLATFORM_Sina];
            [[NSNotificationCenter defaultCenter] postNotificationName:K_UPDATEUSER_NOTE object:nil];
