...
/*
- (void)addObservers{
    [ZCSNotficationMgr addObserver:self call:@selector(didNetDataOK:) msgName:kZCSNetWorkOK];
    [ZCSNotficationMgr addObserver:self call:@selector(di:) msgName:
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (cell == nil) 
    {
        ...
        cell = [LabelFieldCell getCellFromNibFile];
	}
	...
	switch (index)
    {
        case 0:
            ...
            if(!nickname)
            {
             ...
             ((LabelFieldCell *)cell).splitLineView.backgroundColor = kLoginAndSignupCellLineColor;
...
-(BOOL)checkResignInput
{
    ...
    else
    {
        ...
        if ([uname length] > 30) {
            [self alertMsg:NSLocalizedString(@"昵称过长，最多30个字符", @"")];
            [cell.cellField becomeFirstResponder];
            return NO;
        }
    ...
    if ([pass length] > 16) {
        [self alertMsg:NSLocalizedString(@"密码过长，最多16个字符", @"")];
        [cell.cellField becomeFirstResponder];
        return NO;
    }
    [self.resignData setValue:pass  forKey:@"pass"];
...
-(void)resignNow:(id)sender
{
    ...
    */
    ...
    self.request = [netClientMgr  startUserResign:self.resignData];
}
-(void)didLoginOK:(NSNotification*)ntf
{
   
    id obj = [ntf object];
    id repRequest = [obj objectForKey:@"request"];
    id data = [obj objectForKey:@"data"];
    NSString *resKey = [repRequest resourceKey];
    NSString *account =  [self.resignData objectForKey:@"email"];
    if(self.request == repRequest &&[resKey isEqualToString:@"register"])
    {
        kNetEnd(self.view);
        ...
        {
            [[NSUserDefaults standardUserDefaults] setObject:self.imagePath forKey:kUserDefaultIconPath];
